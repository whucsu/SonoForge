...
## 1. 🎨 Цветовая схема — уточнение палитры

**Проблема:** Текущая тёмная тема использует холодный сине-серый (`#0a1018`), который при длительной работе утомляет глаза. Акцентный синий (`#3d7cb8`) плохо различим на тёмном фоне.

### Изменения палитры (dark)

| Токен | Сейчас | → Станет | Обоснование |
|---|---|---|---|
| `bg_dark` | `#0a1018` | `#111827` | Тёплый тёмно-серый (как JetBrains Darcula) — меньше устают глаза |
| `bg_panel` | `#121a24` | `#1a2332` | Чёткое отделение от фона |
| `bg_control` | `#1a2430` | `#243044` | Более заметные контролы |
| `accent` | `#3d7cb8` | `#60a5fa` | Ярче, соответствует WCAG AAA на тёмном |
| `text` | `#e8eef4` | `#f1f5f9` | Теплее, лучше читается |
| `text_dim` | `#8fa3b8` | `#94a3b8` | Без изменений (уже хорошо) |

### Изменения палитры (light)

| Токен | Сейчас | → Станет |
|---|---|---|
| `text` | `#1a1a1a` | `#0f172a` |
| `accent` | `#2563a8` | `#3b82f6` |
| (панели) | `#ffffff` | `#ffffff` + border `#e2e8f0` |

### Анимация смены темы

- Сейчас: мгновенная замена `setStyleSheet()` — глаз не успевает адаптироваться
- Добавить `QGraphicsOpacityEffect` на `centralWidget`: fade 150ms между темами

**Файлы:** `src/echo_personal_tool/presentation/echopac_theme.py`
