...
if [[ "${1:-}" == "--clean" ]]; then
    ...
    rm -rf "${BUILD_DIR}" "${DIST_DIR}/echo-personal-tool" "${VENV_DIR}"
fi
...
echo "[step 3] Running PyInstaller (this may take a few minutes)..."
pyinstaller \
    --name "${APP_NAME}" \
    --onedir \
    --noconfirm \
...
# ── 4. Assemble .deb directory structure ──
echo "[step 4] Assembling .deb package..."
...
# Desktop entry
cp scripts/echo-personal-tool.desktop "${DEB_PKG}/usr/share/applications/"
...
if command -v rsvg-convert &>/dev/null; then
    ...
    sed -i "s|Icon=${APP_NAME}|Icon=/usr/share/icons/hicolor/scalable/apps/${APP_NAME}.svg|" \
        "${DEB_PKG}/usr/share/applications/echo-personal-tool.desktop"
fi
...
Maintainer: ECHO2026 Team
Description: Personal desktop echocardiography analysis tool
 ECHO Personal Tool is a desktop application for echocardiography
 analysis, DICOM viewing, cardiac measurements, and reference management.
EOF

# ── 6. Post-install script (optional: create symlink) ──
cat > "${DEB_PKG}/DEBIAN/postinst" << 'POSTINST'
#!/bin/bash
set -e
# Make the binary executable
chmod +x /opt/echo-personal-tool/echo-personal-tool
# Create a symlink in /usr/local/bin
ln -sf /opt/echo-personal-tool/echo-personal-tool /usr/local/bin/echo-personal-tool
# Update icon cache
...
#!/bin/bash
set -e
rm -f /usr/local/bin/echo-personal-tool
PRERM
chmod 755 "${DEB_PKG}/DEBIAN/prerm"

# ── 7. Build .deb ──
echo "[step 5] Building .deb..."
